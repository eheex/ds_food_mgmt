...
window.$PrdRegistPopup = {
	...
	_onRequest:function(){
		...
		if(_this._onUpload() == "1"){
			...
			/*_data.foodRest = [{"imgID":"test", "imgNM":"testNm", "imgPth":"asdasd/sdsdsd.gif"},
				{"imgID":"test2", "imgNM":"testNm2", "imgPth":"asdasd/sdsdsd.gif"}];*/
			_data.foodRest = [];
			
			$.each(_this.rtFile, function(key,value){
				$.each(_this.rtFile.key, function(a,b){
					console.log("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"+a+":::"+b);
				});
				console.log("key@@@@@@@@@@@@@@@"+key);
				...
				_data.foodRest.push(_fileInfo);*/
			});
			
				/*var file = {"0":this.tagNm,
							"1":this.1,
							"2":this.2};
				}
				_data.foodRest.push(file);*/
			
			$.ajax({
				url: "/portal/prdReq/insert",
				type: "POST",
				data : JSON.stringify(_data),
				/*data: JSON.stringify({"prdNm":prdname,
						    "comNa":brand,
						    "reqGb":"I",
						    "email":email})
			            //,"foodRest" : JSON.stringify(_this.rtFile)
			*/
