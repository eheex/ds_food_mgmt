		...
		this._jFoodTable = $("#prdListTable").DataTable({
		    ...
		    rowCallback : function( row, data, index ) {
		    	  ...
		    	  if(typeof data.notAdd != "undefined" && data.notAdd != ""){
		    		  _notAddTxt = data.notAdd;
		    	  }
		    	  /* 영양영분주의 처리 */
		    	  var _ntrIgdNmTxt = "";
		    	  if(typeof data.ntrIgdNm != "undefined" && data.ntrIgdNm != ""){
		    		  _ntrIgdNmTxt = data.ntrIgdNm;
		    	  }
		    	  ...
		    	  if(typeof data.tag != "undefined" && data.tag != ""){
		           ...
		           $(row).data("rowData", data).append($('<img src="'+_imgUrl+'" alt="메인제품썸네일">'+
			           	  '<div class="info2">'+
							'<div class="info2-1">'+
								'<span class="mark1">알레르기</span>'+
								'<div class="infodetail">원재료에 알레르기성분이 포함되어 있습니다.<br><strong>포함 성분: '+_allrgyTxt+'</strong></div>'+
								'<span class="mark2">인증</span>'+
								'<div class="infodetail">이 제품은 인증 인증을 받았습니다.<br><strong>'+_certTxt+'</strong></div>'+
								'<span class="mark3">무첨가</span>'+
								'<div class="infodetail">이 제품은 무첨가 마케팅을 하고 있습니다.<br><strong>무첨가: '+_notAddTxt+'</strong></div>'+
								'<span class="mark4">영양성분주의</span>'+
								'<div class="infodetail">해당 제품의 '+_ntrIgdNmTxt+'은(는) 1일 영양성분 기준치에 50%를 넘습니다.</div>'+
							'</div>'+
						   ...
						   '<div class="info3">'+
							...
							}).on("mouseover", ".mark1", function(event){
								sleft = -12;
								$(".prd_list1 .info2-1 .mark1+.infodetail").after().css('left',sleft+'%');
							}).on("mouseover", ".mark2", function(event){
								console.log($(this).position());
								
								var sleft = 14;
								if($(this).position().left < 48){
									sleft = -2;	
								}
								
								$(".prd_list1 .info2-1 .mark2+.infodetail").after().css('left',sleft+'%');
							}).on("mouseover", ".mark3", function(event){
								console.log($(this).position());
								
								var sleft = 16;
								if($(this).position().left <= 28){ //알레르기 X 인증 O
									sleft = 0;	
								}else if($(this).position().left <= 48){ //알레르기 O 인증 X
									sleft = 7;
								}
								
								$(".prd_list1 .info2-1 .mark3+.infodetail").after().css('left',sleft+'%');
							}).on("mouseover", ".mark4", function(event){
								console.log($(this).position());
								
								var sleft = 20;
								if($(this).position().left <= 10){ //알레르기 X 인증 O 무첨가 O
									sleft = -2;	
								}else if($(this).position().left <= 10){ //알레르기 X 인증 X 무첨가 O
									
								}else if($(this).position().left <= 10){ //알레르기 O 인증 X 무첨가 O
									
								}else if($(this).position().left <= 10){ //알레르기 O 인증 O 무첨가 X
									
								}else if($(this).position().left <= 10){ //알레르기 X 인증 O 무첨가 O
									
								}
								
								$(".prd_list1 .info2-1 .mark4+.infodetail").after().css('left',sleft+'%');
							})
					);
